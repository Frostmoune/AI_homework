...
restaurantDistrict(Res, Dis):-restaurant(B, _, Res), 
                            branch(B, C), 
                            district(C, Dis).
lessBranch(A, Length):-setof(B, branch(A, B), L1), 
                findall(C, (setof(D, branch(C, D), L2), C\=A, length(L1, E), length(L2, F), F < E), L3), 
                length(L3, Length).
calArea(A):-setof(B, branch(B, A), L), length(L, E), E>=2.
longerHistory(A, Length):-restaurant(A, L1, _), 
                            findall(C, (restaurant(C, L2, _), C\=A, L1 > L2), L3), 
                            length(L3, Length).
calBranch(A):-setof(B, branch(A, B), L), length(L, E), E>=10.

sameDistrict(A, B):-branch(A, C), branch(B, D), 
                    district(C, E), district(D, E), A\=B.