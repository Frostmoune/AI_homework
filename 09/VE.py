class VariableElimination:
    ...
    orderedListOfHiddenVariables, evidenceList):
        ...
        for var in orderedListOfHiddenVariables:
            var_factors = [factor for factor in factorList if var in factor.varList]
...
class Node:
    ...
    def multiply(self, factor):
        ...
        if len(same_variables) > 0:
            ...
            newList = self.varList[:i] + factor.varList[:j] + self.varList[i:] + factor.varList[j + 1:]
        else:
            for x1, y1 in self.cpt.items():
                for x2, y2 in factor.cpt.items():
                    new_cpt[x1 + x2] = y1 * y2
    ...
    def sumout(self, variable):
        ...
        for x in self.cpt:
            ...
            if now_key not in new_cpt:
                new_cpt[now_key] = self.cpt[x[:i] + '0' + x[i + 1:]] + self.cpt[x[:i] + '1' + x[i + 1:]]
    ...
    def restrict(self, variable, value):
        ...
        #Your code here
        i = self.varList.index(variable)
        new_cpt = dict([(x[:i] + x[i + 1:], y) for x, y in self.cpt.items() if int(x[i]) == value])
        new_var_list = self.varList[:i] + self.varList[i + 1:]
